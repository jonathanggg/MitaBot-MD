...
if (plugin.premium && !isPrems) { 
    // PRIMERA VALIDACIÓN: ¿Es un sub-bot NORMAL (no premium)?
    // Si es un sub-bot y NO está marcado como un bot premium, se le niega el acceso.
    if (this.isSubBot && !this.isPremiumBot) {
        return m.reply(`《✧》El comando *${command}* no está disponible en sub-bots estándar.`);
    }

    // SEGUNDA VALIDACIÓN: Para Bots Principales y Bots Premium.
    // Si la ejecución llega aquí, significa que es un Bot Principal O un Prem-Bot.
    // Para ambos, se aplica la misma lógica de 'isPrems', que ya incluye
    // el premium del usuario, el premium de grupo y si es owner.
    if (!isPrems) {
        fail('premium', m, this); // Usa el mensaje de fallo por defecto para 'premium'.
        continue;
    }
}
